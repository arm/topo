...
func Docker(h ssh.SSHConnection, args ...string) *exec.Cmd {
	cmdArgs := append(hostToArgs(h), args...)
	return exec.Command("docker", cmdArgs...)
}

func DockerCompose(h ssh.SSHConnection, composeFile string, args ...string) *exec.Cmd {
...
func hostToArgs(h ssh.SSHConnection) []string {
	if h.IsLocalhost() {
		return nil
	}
